...
model = dict(
    ...
    cls_head=dict(type='GCNHead', num_classes=60, in_channels=256, dropout=0.2))

dataset_type = 'PoseDataset'
ann_file = 'data/skeleton/caregiver_3d.pkl'
train_pipeline = [
...
val_pipeline = [
    ...
    dict(type='GenSkeFeat', dataset='nturgb+d', feats=['jm']),
    dict(
        type='UniformSampleFrames', clip_len=500, num_clips=1, test_mode=True),
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='PackActionInputs')
]
test_pipeline = [
    ...
    dict(type='GenSkeFeat', dataset='nturgb+d', feats=['jm']),
    dict(
        type='UniformSampleFrames', clip_len=500, num_clips=10,
        test_mode=True),
    dict(type='PoseDecode'),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='PackActionInputs')
